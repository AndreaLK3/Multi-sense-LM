...
def create_standardLM_model(objects, include_globalnode_input, use_gold_lm, use_transformer_lm):
...
def create_model(model_type, standardLM_model, objects, K, context_method, C, dim_qkv):
    ...
    if model_type.lower() == 'rnn':
        ...
        model = MFS.MFS(StandardLM, graph_dataobj, grapharea_size, grapharea_matrix,
                 vocabulary_df, batch_size=32, n_layers=3, n_hid_units=1024, K=1, mfs_df=mfs_df)
    else:
...
def setup_pretraining_on_WT2(include_globalnode_input, use_gold_lm, use_transformer_lm,
                             batch_size, seq_len,
    ...
    standardLM_model = create_standardLM_model(objects, include_globalnode_input, use_gold_lm, use_transformer_lm)
...
def setup_training_on_SemCor(standardLM_model, model_type=None, K=1, context_method_id=0, C=20,
    ...
    model = create_model(model_type, standardLM_model, objects, K, context_method, C, dim_qkv)
    model.to(DEVICE)
