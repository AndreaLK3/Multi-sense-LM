...
class WD_LSTM(torch.nn.Module):
    def __init__(self, data, grapharea_size, include_senses, batch_size, n_layers, n_units):
        ...
        self.lstm = C.weight_drop(module=torch.nn.LSTM(input_size=self.d, hidden_size=n_units, num_layers=n_layers),
...
class LSTM(torch.nn.Module):
    ...
    def forward(self, batchinput_tensor):  # given the batches, the current node is at index 0
        ...
        if self.include_senses:
            ...
            predictions_senses = torch.tensor([0] * self.batch_size * seq_len).to(
                DEVICE)  # so I don't have to change the interface elsewhere
