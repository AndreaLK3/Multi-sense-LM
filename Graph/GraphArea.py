...
def get_grapharea_elements(starting_node_index, area_size, graph):
    ...
    # area_x = graph.x.index_select(0, node_indices) -- Not used in this version, where I store the adjacencies
    ...
    # with torch.no_grad():
    #     area_edge_index = torch.autograd.Variable(torch.Tensor(edge_index_lts_defaultValues).t().to(torch.int64)).to(DEVICE)
