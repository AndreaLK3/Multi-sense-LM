...
class Ensemble_Combine(nn.Module):
    ...
    def __init__(self, AWD_base, AWD_modified):
        ...
        self.concatenated_encoding_dim = AWD_base.ninp + AWD_modified.ninp
...
class AWD(nn.Module):
    ...
    def init_hidden(self, bsz):
        weight = next(self.parameters()).data
        print("AWD>init_hidden_weight=" + str(weight))
        if self.rnn_type == 'LSTM':
...
# the original AWD-LSTM is unchanged
class AWD_empty(nn.Module):
    """Container module with an encoder, a recurrent module, and a decoder."""

    def __init__(self, rnn_type, ntoken, ninp, nhid, nlayers,
                 dropout=0.5, dropouth=0.5, dropouti=0.5,
                 dropoute=0.1, wdrop=0, tie_weights=False):
        super(AWD_empty, self).__init__()
        self.lockdrop = LockedDropout()
        self.idrop = nn.Dropout(dropouti)
        self.hdrop = nn.Dropout(dropouth)
        self.drop = nn.Dropout(dropout)
    ...
    def forward(self, input, hidden, return_h=False):
        ...
        hidden = hidden
        raw_outputs = torch.zeros(size=(10,))
        outputs = torch.zeros(size=(10,))
        if return_h:
            return result, hidden, raw_outputs, outputs
        else:
            return result, hidden
    ...
    def init_hidden(self, bsz):
        weight = next(self.parameters()).data
        print("AWD_empty>init_hidden_weight=" + str(weight))
        if self.rnn_type == 'LSTM':
            return [(weight.new(1, bsz, self.nhid if l != self.nlayers - 1 else (self.ninp if self.tie_weights else self.nhid)).zero_(),
                    weight.new(1, bsz, self.nhid if l != self.nlayers - 1 else (self.ninp if self.tie_weights else self.nhid)).zero_())
                    for l in range(self.nlayers)]
