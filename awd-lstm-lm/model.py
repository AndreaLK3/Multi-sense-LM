...
class AWD_modified(nn.Module):
    ...
    def __init__(self, rnn_type, ntoken, nhid, nlayers, graph_dataobj, variant_flags_dict, my_vocabulary_wordlist,
        ...
        self.encoder = nn.Embedding(ntoken, self.d_inp * (1 + sum([int(flag) for flag in variant_flags_dict])))
        self.ntokens_vocab = ntoken
        assert rnn_type in ['LSTM', 'QRNN', 'GRU'], 'RNN type is not supported'
        if rnn_type == 'LSTM':
            self.rnns = [torch.nn.LSTM(self.d_inp if l == 0 else nhid, nhid if l != nlayers - 1 else (self.d_inp if tie_weights else nhid), 1, dropout=0) for l in range(nlayers)]
            if wdrop:
                self.rnns = [WeightDrop(rnn, ['weight_hh_l0'], dropout=wdrop) for rnn in self.rnns]
        if rnn_type == 'GRU':
            self.rnns = [torch.nn.GRU(self.d_inp if l == 0 else nhid, nhid if l != nlayers - 1 else self.d_inp, 1, dropout=0) for l in range(nlayers)]
            if wdrop:
                self.rnns = [WeightDrop(rnn, ['weight_hh_l0'], dropout=wdrop) for rnn in self.rnns]
        # elif rnn_type == 'QRNN':
    ...
    def forward(self, input, hidden, return_h=False):
        ...
        for t in range(seq_len):
            if not (self.variant_flags_dict['include_globalnode_input']):
                ...
                pass # lemmatization + global node state will be added here
