...
def compute_MFS_for_corpus_previous():
    ...
    corpus_fpath = os.path.join(F.FOLDER_TEXT_CORPUSES, F.FOLDER_SENSELABELED)
    ...
    for b_idx in range(len(train_dataloader)-1):
        ...
        input_sense = get_sense_from_idx(senseindices_db_c, input_sense_index)
    ...
    for w_idx in range(len(mfs_vocabulary_ls)):
        ...
        try:
            ...
            mfs = get_sense_from_idx(senseindices_db_c, mfs_idx)
